...
const PROD_API_TOKEN = process.env.PROD_STRAPI_API_TOKEN || process.argv[2];
const EXPORT_FILE = process.env.EXPORT_FILE || path.join(__dirname, "properties-export.json");

if (!PROD_API_TOKEN) {
  console.error("❌ Production API token is required");
  console.error("   Usage: node seed/import-properties-to-prod.js <api-token>");
  console.error("   Or set PROD_STRAPI_API_TOKEN environment variable");
  process.exit(1);
}
...
async function uploadMediaToProd(mediaUrl, fileName, localStrapiUrl = "http://localhost:1337") {
  if (!mediaUrl || !fileName) return null;

  try {
    // Download file from local
    const fullUrl = mediaUrl.startsWith('http') ? mediaUrl : `${localStrapiUrl}${mediaUrl}`;
...
async function createPropertyInProd(property, localStrapiUrl = "http://localhost:1337") {
  ...
  if (attributes.images?.data && Array.isArray(attributes.images.data)) {
    ...
    for (const image of attributes.images.data) {
      ...
      if (imageUrl) {
        ...
        const uploadedId = await uploadMediaToProd(imageUrl, imageName, localStrapiUrl);
        if (uploadedId) {
  ...
  if (attributes.documents?.data && Array.isArray(attributes.documents.data)) {
    ...
    for (const doc of attributes.documents.data) {
      ...
      if (docUrl) {
        ...
        const uploadedId = await uploadMediaToProd(docUrl, docName, localStrapiUrl);
        if (uploadedId) {
