...
const transportLinks = ["Elizabeth Line access", "Greater Anglia rail services", "Road connectivity via A12 and A127"];
  ...
  return (
    <main className="bg-white text-[#374151]">
      <section className="relative min-h-[500px] overflow-hidden sm:min-h-[560px] lg:min-h-[680px]">
        <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: "url('/images/property-management-essex/hero-img-property-management-in-essex-v2.png')" }} />
        ...
        <div className="relative mx-auto flex min-h-[500px] max-w-[1440px] items-center justify-center px-5 text-center sm:min-h-[560px] 650:px-[60px] lg:min-h-[680px] lg:px-[40px] 1100:px-[80px] 1300:px-[100px]">
          <div className="mt-10 max-w-3xl">
            ...
            <p className="mx-auto mt-6 max-w-2xl font-manrope text-[15px] leading-[26px] text-white/85 sm:text-[16px] sm:leading-[28px]">
              Pevona offers property management services across Essex, helping landlords manage properties efficiently and in line with current regulations.
            </p>
      ...
      <section className="bg-[#f9fafb] py-16 sm:py-20 lg:py-24">
        <div className="mx-auto max-w-[1440px] px-5 650:px-[60px] lg:px-[40px] 1100:px-[80px] 1300:px-[100px]">
          <div className="mx-auto mb-14 max-w-3xl text-center">
            <h2 className="font-crimson text-[32px] leading-tight text-[#061e38] sm:text-[42px] lg:text-[48px]">Our Services in Essex</h2>
            <p className="mt-4 font-manrope text-[15px] leading-[26px] text-[#6b7280] sm:text-[16px] sm:leading-[28px]">We help Essex landlords manage property responsibilities through structured management, compliance support, and ongoing tenant coordination.</p>
          </div>
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3">
            {services.map((service) => (
              <Link key={service.title} href={service.href} className="block rounded-[24px] bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-lg overflow-hidden">
        ...
        <div className="mx-auto max-w-[1320px] px-5 650:px-[60px] lg:px-[40px] 1100:px-[80px] 1300:px-[100px]">
          <div className="grid grid-cols-1 items-center gap-10 lg:grid-cols-2 lg:gap-20">
            <div className="order-2 overflow-hidden rounded-[24px] shadow-2xl lg:order-1">
              <div className="h-[420px] w-full bg-cover bg-center sm:h-[500px] lg:h-[560px]" style={{ backgroundImage: "url('/images/property-management-essex/areas-we-cover-v2.png')" }} />
            </div>
            <div className="order-1 space-y-10 lg:order-2">
              <div>
                ...
                <ul className="mt-5 grid grid-cols-1 gap-3 text-[15px] text-white/90 sm:grid-cols-2">
                  {coveredAreas.map((area) => (<li key={area} className="flex items-center gap-3"><span className="h-[7px] w-[7px] rounded-full bg-[#2dd4bf]" />{area}</li>))}
                </ul>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="mt-4 rounded-[18px] border border-white/10 bg-white/5 p-5 font-manrope text-[15px] leading-[26px] text-white/80">Essex continues to attract tenants due to affordability and strong commuter links into London.</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="grid gap-8 lg:grid-cols-[0.95fr_1.05fr]">
            <div className="rounded-[30px] border border-[#e5e9ef] bg-[#f9fafb] p-7 sm:p-8">
              ...
              <div className="mt-6 grid gap-3 sm:grid-cols-2">
                {trustLinks.map((link) => (<Link key={link.href} href={link.href} className="rounded-[18px] border border-[#dbe2ea] bg-white px-4 py-4 font-manrope text-[14px] font-medium text-[#1f2937] transition hover:border-[#cfd8e3]">{link.name}</Link>))}
              </div>
            </div>
            <div className="rounded-[30px] border border-[#e5e9ef] bg-[#f9fafb] p-7 sm:p-8">
      ...
      <section className="border-b border-[#e5e7eb] bg-[#f9fafb] py-16 sm:py-20">
        <div className="mx-auto max-w-4xl px-5 text-center 650:px-[60px] lg:px-[40px]">
          <h2 className="font-crimson text-[32px] leading-tight text-[#061e38] sm:text-[40px]">Book your free valuation today.</h2>
          <div className="mt-8 flex flex-wrap items-center justify-center gap-3">
            <Link href="/contact" className="inline-flex min-h-[54px] items-center justify-center rounded-full bg-[#061e38] px-8 py-3 text-[14px] font-semibold text-white transition hover:bg-[#031224]">Book a Free Valuation</Link>
            <Link href="/areas-we-cover" className="inline-flex min-h-[54px] items-center justify-center rounded-full border border-[#d7dde6] bg-white px-8 py-3 text-[14px] font-semibold text-[#061e38] transition hover:bg-[#f7f8fa]">View All Areas</Link>
          </div>
        </div>
      </section>
    </main>
